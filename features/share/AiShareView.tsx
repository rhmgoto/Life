...
export function AiShareView({ logs, events }: { logs: LogEntry[]; events: ScheduleEvent[] }) {
  ...
  const includedEvents = useMemo(() => events.filter((event) => scope !== 'selected' && event.date >= range.from && event.date <= range.to), [events, scope, range]);
  const output = useMemo(() => formatAiShare(includedLogs, includedEvents, range.from, range.to), [includedLogs, includedEvents, range]);
