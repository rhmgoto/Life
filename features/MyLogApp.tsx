...
import { SyncedLogRepository, type SyncStatus } from '@/data/syncedLogRepository';
...
export function MyLogApp() {
  ...
  const [syncStatus, setSyncStatus] = useState<SyncStatus>({ state: 'synced', pendingCount: 0 });
  ...
  const visibleLogs = useMemo(() => data.logs.filter((log) => !log.deletedAt), [data.logs]);
  const dayLogs = useMemo(() => visibleLogs.filter((log) => log.date === date).sort((a, b) => a.time.localeCompare(b.time)), [visibleLogs, date]);
  ...
  const storageLabel = session
    ? syncStatus.state === 'syncing' ? '同期中…'
      : syncStatus.pendingCount > 0 ? `未同期 ${syncStatus.pendingCount}件`
        : syncStatus.state === 'offline' ? '端末に保存・再接続待ち' : 'クラウド同期済み'
    : 'この端末に保存中';

  return <main className="app-shell">
    <AppNavigation view={view} storageLabel={storageLabel} onChange={(next) => { setView(next); if (next === 'today') setDate(toDateKey()); }} />
    <section className="content">
      <button className={`sync-chip ${syncStatus.state}`} onClick={() => setSettingsOpen(true)} aria-label="保存と同期の設定を開く"><span>●</span>{storageLabel}<b>⋯</b></button>
      <>
        {view === 'today' && <TodayView date={date} logs={dayLogs} onDateChange={setDate} onNewLog={() => setEditingLog('new')} onEditLog={setEditingLog} />}
        {view === 'calendar' && <CalendarView selectedDate={date} logs={visibleLogs} onMonthChange={setDate} onOpenDate={openDay} />}
        {view === 'search' && <SearchView logs={visibleLogs} onOpenDate={openDay} onSaveLog={updateLog} />}
        {view === 'share' && <AiShareView logs={visibleLogs} />}
      </>
    </section>
    ...
    {settingsOpen && <DataSettings data={{ ...data, logs: visibleLogs }} repository={repository} user={session?.user ?? null} configured={configured} syncStatus={syncStatus} onSyncNow={refresh} onClose={() => setSettingsOpen(false)} onImported={refresh} onSignOut={async () => { await getSupabaseClient().auth.signOut(); setSettingsOpen(false); }} />}
  </main>;
}
